...
export default class FormioDownloadProjectFormsPlugin extends PureComponent {
  ...
  async handleConfigClosed(config) {
    if (config) {
      const token = await this.formioLogin(
        config.loginUrl,
        config.userName,
        config.password,
      );
      const forms = await this.formioFormsByTag(
        token,
        config.projectUrl,
        config.tags,
      );
  ...
  async getTabConfiguration(tab) {
    const credentials = await this.props.config.getForFile(
      tab.file,
      "formio-designer-credentials",
    );
    const data = await this.props.config.getForFile(
      tab.file,
      "formio-designer-download-form",
    );
    return {
      ...credentials,
      ...data,
    ...
    return (
      <Fragment>
        <Fill slot="toolbar" group="9_formioDesigner">
          <button
            type="button"
            onClick={(event) => this.handleOpenDownloadDialog(event)}
          >
            Form.io Forms
          </button>
        </Fill>
