...
export default function FormioFormSelect({ config, onClose }) {
  const onSubmit = () => onClose({ formId, fileName, varName, inputDS, outputDS });
  ...
  for (let property in forms) {
    items.push(<option key={ property } value={ property }>{ forms[property].name }</option>);
  }

  let sources = config.dataSources.map(ds => <option key={ ds.id } value={ ds.id }>{ ds.name }</option>);

  return <Modal id="modalSelect" className="form-builder" onClose={ onClose }>
                  ...
                  <td>
                    ...
                    <div style={ selectStyle }>
                      <select className="form-control"
                        name="formSelectControl" value={ formId }
    ...
    <Footer>
      <div id="dialogButtons" className="form-submit">
        <button type="submit" class="btn btn-primary" form="formSelect">Select</button>
