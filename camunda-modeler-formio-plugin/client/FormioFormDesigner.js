...
export default function FormioFormDesigner({ config, onClose }) {
  ...
  let sources = config.dataSources.map(ds => <option key={ ds.id } value={ ds.id }>{ ds.name }</option>);

  return <Modal id="formBuilder" className="form-builder">
    ...
    <Body>
            ...
            <div className="form-group">
              <div>
                <input type="file"
                       ...
                       accept=".formio"
                />
                <label className="btn btn-secondary" htmlFor="formFileInput">Browse...</label>
              </div>
              <div>
              ...
              { showInput && (<div>
                <label htmlFor="inputSubmissionSelectControl">Input data source</label>
                <select className="form-control"
                        name="inputSubmissionSelectControl" value={ inputDS }
                        onChange={ (event) => setInputDS(event.target.value) }>
                  { sources }
                </select>
              </div>)}
              <div>
                <label htmlFor="outputSubmissionSelectControl">Output data source</label>
                <select className="form-control"
                        name="outputSubmissionSelectControl" value={ outputDS }
      ...
      <div class="bootstrap-iso">
        <FormBuilder form={originSchema} onChange={(schema) => formSchema = schema} options={options} />
      </div>
    </Body>

    <Footer>
      <div id="dialogButtons">
        <button type="submit" class="btn btn-primary" onClick={() => onClose({formSchema, formName, varName, inputDS, outputDS})}>Save</button>
