...
class FileService {
  ...
  async xhrRequest(method, url, name, query, data, options, progressCallback, abortCallback) {
  ...
  async uploadFile(storage, file, fileName, dir, evt, url, options, progressCallback, abortCallback) {
    console.log("FileService Upload file: ", storage, file, fileName, dir, url, options);

    return this.xhrRequest('POST', url, fileName, {}, {
      file: file,
      name: fileName,
      dir
  ...
  async downloadFile(fileInfo, options) {
    ...
    return this.xhrRequest('GET', fileInfo.url, fileInfo.name, {}, {})
