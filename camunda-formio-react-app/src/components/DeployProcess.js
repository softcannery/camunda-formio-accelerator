...
class DeployProcess extends Component {
  ...
  render() {
    if (!this.props.processDeployment) {
      return (
        <Form>
          <FileReaderInput as="binary" id="my-file-input" onChange={this.handleFileReaderInput} multiple>
            <Button primary>Browse Files to Deploy BPMN and Formio forms</Button>
          </FileReaderInput>
        </Form>
      )
    } else {
